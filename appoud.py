...
def render_badges_csv(csv_text: str) -> str:
    """Zet 'ik, partner, dochter' om naar HTML-badges. Lege entries worden genegeerd."""
...
st.title("🍳 Recepten & Eetlijst (MVP)")
...
with st.sidebar:
    ...
    r_ingredients = st.text_area("Ingrediënten (1 per regel)")
    r_instructions = st.text_area("Bereiding / stappen")
    r_tags = st.text_input("Tags/labels (komma-gescheiden)", placeholder="pasta, snel, kids")
    r_liked_by = st.text_input("Wie vindt dit lekker? (komma-gescheiden)", placeholder="ik, partner, zoon, dochter")
    if st.button("Opslaan", type="primary", use_container_width=True):
        if r_name.strip():
            add_recipe(r_name.strip(), r_ingredients.strip(), r_instructions.strip(), r_tags.strip(), r_liked_by.strip())
...
if not rows:
    ...
    for rid, name, ingredients, instructions, tags, liked_by in rows:
        with st.container(border=True):
            ...
            tabs = st.tabs(["Ingrediënten", "Bereiding", "Eetmoment loggen", "Verwijderen"])
            with tabs[0]:
